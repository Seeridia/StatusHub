...
品牌清单位于 `web/src/lib/brands.ts`，图标位于 `web/public/brands/`。图标来源及版本见 [品牌素材来源](brand-assets.md)。页面不加载第三方 Logo CDN；图标保留明确尺寸并懒加载，窄屏使用多行网格，品牌名始终可见。修改品牌清单时需同步图标、来源和展示数量，再构建网页并重启 API。
