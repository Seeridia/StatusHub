...
| ------------------------------------------- | ------ | -------- | ----- |
| 查看可见服务、事件、规则、渠道、投递        | 是     | 是       | 是    |
| 添加与归档来源、管理规则/渠道、测试与重试   | 否     | 是       | 是    |
| 审计导出、成员、邀请和服务账号管理          | 否     | 否       | 是    |
| 转移管理员                                  | 否     | 否       | 是    |

每个工作区只有一名有效人员 Admin。平台适配器发布、共享采集器维护和区域故障转移属于可信运维能力，不由工作区角色开放。CLI 直接使用数据库权限，仅交给可信运维使用。

创建服务账号使用 `service-account-create -tenant-id ... -name ... -role ...`，角色仅允许 `operator` 或 `viewer`。创建结果中的 `token` 是 API 凭据，格式以 `sa.` 开头；丢失后应轮换。完整流程见[团队账号管理](team-accounts.md)。
