...
func testDatabase(t *testing.T) string {
	...
	name := "statusmon_deploy_test_" + strings.ReplaceAll(time.Now().Format("150405.000000000"), ".", "")
...
func TestMigrationLifecycle(t *testing.T) {
	...
	var count int
	if err = conn.QueryRow(ctx, "SELECT count(*) FROM public.statusmon_schema_migrations").Scan(&count); err != nil {
		t.Fatal(err)
	}
	if count != 13 {
	...
	if _, err = conn.Exec(ctx, "INSERT INTO statusmon_schema_migrations(name,checksum) VALUES('999999_future.up.sql','x')"); err != nil {
		t.Fatal(err)
	}
...
func TestMigrationFailureIsAtomic(t *testing.T) {
	...
	var count int
	if err = conn.QueryRow(ctx, "SELECT count(*) FROM statusmon_schema_migrations").Scan(&count); err != nil || count != 0 {
