// statusmon-migrate applies new schema migrations with checksums and a shared lock.
package main
...
import (
	...
	"github.com/vendor-status-monitoring/vendor-status-monitoring/migrations"
)
...
func run(ctx context.Context, url string, files fs.FS) error {
	...
	var tracked bool
	if err := conn.QueryRow(ctx, "SELECT to_regclass('public.statusmon_schema_migrations') IS NOT NULL").Scan(&tracked); err != nil {
		return err
	}
	if !tracked {
		var occupied bool
		...
		if _, err := conn.Exec(ctx, "CREATE TABLE public.statusmon_schema_migrations (name text PRIMARY KEY, checksum text NOT NULL, applied_at timestamptz NOT NULL DEFAULT now())"); err != nil {
			return err
		}
	}
	...
	rows, err := conn.Query(ctx, "SELECT name, checksum FROM public.statusmon_schema_migrations")
	if err != nil {
		return err
	}
	...
	for _, n := range names {
		...
		if _, err = tx.Exec(ctx, sql); err == nil {
			_, err = tx.Exec(ctx, "INSERT INTO public.statusmon_schema_migrations (name,checksum) VALUES ($1,$2)", n, fmt.Sprintf("%x", sha256.Sum256(data)))
		}
		if err != nil {
			_ = tx.Rollback(ctx)
