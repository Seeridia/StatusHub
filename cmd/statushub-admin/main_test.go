...
func TestRunIdentityAndAuditCommands(t *testing.T) {
	...
	if err := run(context.Background(), append([]string{"oidc-provider-upsert"}, append(common,
		"-issuer", "https://id.example.com", "-client-id", "statushub", "-allowed-domains", "example.com")...), io.Discard); err != nil {
		t.Fatal(err)
	}
	if fake.oidcProvider.ClientID != "statushub" || len(fake.oidcProvider.AllowedDomains) != 1 {
